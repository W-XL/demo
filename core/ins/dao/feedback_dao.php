...
class feedback_dao extends Dao{
    ...
    public function get_fag($app_id){
        ...
        if(!$data){
            ...
            $this->doResultList();
            $data =  $this->result;
            memcache_set($this->mmc, "feedback_faq_".$app_id, $data, MEMCACHE_COMPRESSED, 600);
        }
        return $data;
    }

    public function get_my_problem($app_id, $usr_id, $page = 0, $num=5) {
        $data = memcache_get($this->mmc, 'sdk_feedback_info_'.$usr_id.'_'.$app_id);
        if(!$data) {
            $this->sql = 'select id,content,feedback,read_status from sys_feedbacktb
                where user_id=? and appid=? and is_del=0 order by create_time desc';
            $this->params = array($usr_id, $app_id);
            $this->doResultList();
            $data = $this->result;
            memcache_set($this->mmc, 'sdk_feedback_info_'.$usr_id.'_'.$app_id, $data, MEMCACHE_COMPRESSED, 600);
        }
        if($data) {
...
//        $this->sql = "select servname,nickname,playerid from sys_common_game_user_tb where pid=? and userid=? and servname!='' group by servid order by add_time";
        $this->sql = "select AreaServerName,RoleName,RoleID  from stats_user_app where AppID=? and UserID=? group by AreaServerID order by  ActTime";
    ...
    public function insert_account_back($data){
        $this->sql = 'insert into sys_account_back(uuid,email,transfer_id, user_info, device,
            pay_order, img_path, creation_time, last_time,`other`,add_time) values(?,?,?,?,?,?,?,?,?,?,?)';
        $this->params = array($data['uuid'],$data['email'], $data['transfer_id'], $data['user_info'], $data['device'],
    ...
    public function get_app_name($app_id){
        ...
        if (!$app_name) {
            ...
            memcache_set($this->mmc, 'help_app_name_' . $app_id, $app_name, MEMCACHE_COMPRESSED, 86400);
        }
        return $app_name;
    }

    public function insert_feedback($data){
        $this->sql = 'insert into sys_feedbacktb(appid, user_id, gamever, net, mtype,
            osname, osver, logintype, sdkver,`server_name`,img_path,
            role_name, player_id, content, create_time, `type`) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
        $this->params = array($data['appid'], $data['usr_id'], $data['gamever'], $data['net'], $data['mtype'],
            ...
            $data['nick_name'], $data['player_id'], $data['content'], date('Y-m-d H:i:s'),$data['type']);
        $this->doInsert();
    ...
    public function set_usr_session($key, $data){
        ...
        memcache_set($this->mmc, "feedback-session-".session_id(), $session_data, MEMCACHE_COMPRESSED, 600);
    }
