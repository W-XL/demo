...
class product_admin_dao extends niuniuDao {
    ...
    public function insert_product($params){
        $this->sql = "insert into app_goods(app_id, good_name, good_code, good_unit, good_amount, good_type,good_intro,good_price,status,rec_type)values(?,?,?,?,?,?,?,?,?,?)";
        $this->params = array($params['app_id'],$params['good_name'],$params['good_code'],$params['good_unit'],$params['good_amount'],$params['good_type'],
                                $params['good_intro'],$params['good_price'],$params['status'],$params['rec_type']);
        $this->doInsert();
        return $this->LAST_INSERT_ID;
    }

    public function update_product($params, $id){
        $this->sql = "update app_goods set good_name=?,good_code=?,good_unit=?,good_amount=?,good_type=?,good_intro=?,good_price=?,status=?,app_id=?,rec_type=? where id=?";
        $this->params = array($params['good_name'], $params['good_code'], $params['good_unit'], $params['good_amount'], $params['good_type'],
                                $params['good_intro'], $params['good_price'], $params['status'], $params['app_id'],$params['rec_type'], $id);
        $this->doExecute();
    }
    ...
    public function import_app_goods($import_data){
        $sql = 'INSERT INTO app_goods(good_name,good_code,good_unit,good_amount,good_type,good_intro,good_price,status,app_id)VALUES';
        foreach ($import_data as $value){
            $sql .= "('".$value['good_name']."','".$value['good_code']."','".$value['good_unit']."',".$value['good_amount'].",".$value['good_type'].",
                    '".$value['good_intro']."',".$value['good_price'].",".$value['status'].",".$value['app_id']."),";
        }
        $this->sql = trim($sql,',');
        $this->params = array();
        $this->doInsert();
